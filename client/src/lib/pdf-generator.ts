...
export async function generatePDF(options: PDFExportOptions): Promise<void> {
  ...
  try {
    ...
    // Ensure proper page ending and spacing
    const currentY = (doc as any).lastAutoTable?.finalY || 150;
    
    // Add final spacing to ensure content doesn't get cut off
    ...
    doc.setFontSize(8);
    doc.setTextColor(150, 150, 150);
    doc.text(`Report generated by ADmyBRAND Insights • ${new Date().toLocaleDateString()}`, 20, footerY);
